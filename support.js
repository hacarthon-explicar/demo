...
(() => {
  ...
  function warnUnresolved(ctx, what) {
    ...
    warnedHoles.add(key);
    console.warn("[runtime] " + (ctx?.__name || "template") + ": " + what);
  }
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      componentDidCatch(e, info) {
        console.error(
          "[runtime] render error in <" + this.__name + ">:",
          e,
          info?.componentStack || ""
        );
      }
  ...
  function createExternalModules(onResolved) {
    ...
    function load(kind, url, after) {
      ...
      cache.set(url, null);
      console.info("[runtime] x-import: loading", url, "(" + kind + ")");
      ...
      const p = ready.then(() => fetch(url)).then((r) => {
        ...
        console.error(
          "[runtime] x-import: FAILED to load",
          url,
          "(" + kind + ")",
          e
        );
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      fetch(url).then((res) => {
        if (!res.ok) {
          console.error(
            "[runtime] sibling fetch for <" + name + "/> failed:",
            url,
            "returned",
            res.status,
        ...
        if (!parsed) {
          console.error(
            "[runtime] sibling fetch for <" + name + "/>:",
            url,
            "has no component template."
          );
          return;
        }
        ...
        (e) => console.error(
          "[runtime] sibling fetch for <" + name + "/> threw:",
          url,
          e
        )
      );
    ...
    function updateHtml(name, html) {
      ...
      try {
        ...
        console.error("[runtime] template compile FAILED for", name, e);
      }
      registry.bump(name);
    }
    function updateJs(name, src) {
      ...
      try {
        ...
        if (r.jsSeq !== seq) return;
        console.error(
          "[runtime] logic class eval FAILED for",
          name,
